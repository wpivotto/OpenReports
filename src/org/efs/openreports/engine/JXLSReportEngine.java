...
import org.efs.openreports.providers.impl.UserProviderImpl;
			...
			throws ProviderException
	{
		...
		{  
			...
			if (report.getQuery() != null && report.getQuery().trim().length() > 0)
			{
				...
				jxlsReportMap.put("date", new Date());
				
				try {
					ReportUser user = (ReportUser) ActionContext.getContext().getSession().get(ORStatics.REPORT_USER);
					jxlsReportMap.put("user", user.getName());
				} catch (NullPointerException e) {
					jxlsReportMap.put("user", "sistema");
				}
			}
			else
			{
